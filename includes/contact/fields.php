...
class zu_ContactFields {
	...
	public function attr($id, $name, $value) {
		...
		if ($name === 'required') {
			$this->fields[$id]['is_required'] = empty($this->fields[$id][$name]) ? false : true;
		}
		return true;
	}
	...
	public function fields($sorting_key = 'order', $template = false, $rows = null) {
		...
		usort($fields, function ($a, $b) use ($sorting_key) {
			return ($a[$sorting_key] == $b[$sorting_key]) ? 0
				: ($a[$sorting_key] > $b[$sorting_key] ? 1 : -1);
		});
	...
	private function data_required($required_value) {
		...
		// NOTE: не нужно, видимо... совсем не использую в JS, осталось наследие от прошлой версии
	...
	private function input($field, $value) {
		...
		$placeholder = empty($field['placeholder']) ? ''
			: sprintf('placeholder="%1$s"', $field['placeholder']);
		$name = $field['type'] === 'submit' ? ''
			: sprintf('name="%2$s[%1$s]"', $field['name'], self::$css_prefix);
	...
	private function textarea($field, $value, $rows = null) {
		return zu_sprintf(
			'<textarea
				class="__zu-control" data-id="%3$s" data-label="%7$s" %1$s 
				id="%6$s-%3$s" name="%6$s[%3$s]" rows="%2$s"%5$s>
				%4$s
			</textarea>',
			$this->data_required($field['required']),
